        ...
        <div className="grid md:grid-cols-6 sm:grid-cols-4 xs:grid-cols-2 2xs:grid-cols-1 mt-20 px-5 gap-4 ">
          {subcategory &&
            subcategory.map((item) => {
              return (
                <div className=" bg-white md:w-48 sm:w-44 mt-5 rounded-2xl shadow cursor-pointer transform transition-all hover:scale-105 ">
                  <NavLink to={`product-details/${item.id}`}>
                    <img
                      className="xs:w-full xs:h-auto md:w-72 md:h-56 rounded-md bg-white"
                      src={item.image}
                      alt="product"
                    />
                  </NavLink>
